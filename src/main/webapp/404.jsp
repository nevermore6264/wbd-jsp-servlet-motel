<%--
  Created by IntelliJ IDEA.
  User: Admin
  Date: 5/25/2018
  Time: 4:56 PM
  To change this template use File | Settings | File Templates.
--%>
<%@ page contentType="text/html;charset=UTF-8" language="java" %>
<html>
<head>
    <title>Find Motel</title>
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta charset="UTF-8">
    <link href="/resources/css/header.css" rel="stylesheet" type="text/css"/>
    <link href="/resources/css/slidershow_index.css" rel="stylesheet" type="text/css">
    <link href="/resources/css/menu_nav.css" rel="stylesheet" type="text/css"/>
    <script src="/resources/js/menu_nav.js" type="text/javascript"></script>
    <script src="/resources/js/jquery-1.11.3.min.js"></script>
    <link href="/resources/css/list_motel.css" rel="stylesheet">
    <link href="/resources/css/footer.css" rel="stylesheet">
</head>
<body>
<jsp:include page="/views/header.jsp"></jsp:include>
<jsp:include page="/views/menu_nav.jsp"></jsp:include>
<jsp:include page="/views/slideshow.jsp"></jsp:include>
<jsp:include page="/views/slide-bar.jsp"></jsp:include>
<article>
    <p style="font-size: 100px">Không có nhà bạn cần tìm trong khoảng tiền bạn vừa nhập !</p>
    <p style="text-transform: inherit;font-size: 70px">Mời bạn nhập lại</p>
</article>
<jsp:include page="/views/footer.jsp"></jsp:include>
</body>
</html>
